...
static const spawn_field_t spawn_fields[] = {
    ...
    // count, on every other entity the arena that entity belongs to.  Keep it
    // when reconciling this table against Q2PRO's.
...
void SP_worldspawn(edict_t *ent)
{
    ...
    // status bar program
    if (deathmatch->value)
        // RA2's dm_statusbar is a complete bar, not a supplement to single_statusbar,
        // so it is not concatenated the way baseq2 does it
